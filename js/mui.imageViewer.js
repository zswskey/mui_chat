...
(function($, document) {
	...
	var ImageViewer = $.ImageViewer = $.Class.extend({
		...
		init: function(selector, options) {
			...
			self.createViewer();
			//图片过长时增加上下滚动
			self.viewer.style.overflowY='scroll';
			self.bindEvent();
		},
		//创建图片预览组件的整体 UI
		createViewer: function() {
			var self = this;
			self.viewer = $.dom("<div class='mui-imageviewer'><div class='mui-imageviewer-mask'></div><div style='position:fixed;-webkit-transform: translateZ(0);' class='mui-imageviewer-header'><i class='mui-icon mui-icon-closeempty mui-imageviewer-close'></i><span class='mui-imageviewer-state'></span></div><i style='position:fixed;-webkit-transform: translateZ(0);' class='mui-icon mui-icon-arrowleft  mui-imageviewer-left'></i><i style='position:fixed;-webkit-transform: translateZ(0);' class='mui-icon mui-icon-arrowright mui-imageviewer-right'></i></div>");
		...
		createImage: function(index, type) {
			...
			item.appendChild($.dom('<span><img style="width: 100%;" src="' + self.images[self.index].src + '"/></span>')[0]);
